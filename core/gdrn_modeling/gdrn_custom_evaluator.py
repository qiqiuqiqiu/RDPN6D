...
from lib.pysixd.pose_error import add, adi, arp_2d, re, te, vsd
from lib.pysixd import inout, misc, renderer
...
class GDRN_EvaluatorCustom(DatasetEvaluator):
    ...
    def __init__(self, cfg, dataset_name, distributed, output_dir, train_objs=None):
        ...
        self.diameters = [self.data_ref.diameters[self.data_ref.objects.index(
    ...
    def _save_vis(self, file_name, cls_name, score, K, bbox_xyxy, R_pred, t_pred):
        ...
        cv2.line(img_vis, origin, tuple(axis_pts_2d[1]), (0, 0, 255), 2, cv2.LINE_AA)  # X
        cv2.line(img_vis, origin, tuple(axis_pts_2d[2]), (0, 255, 0), 2, cv2.LINE_AA)  # Y
        cv2.line(img_vis, origin, tuple(axis_pts_2d[3]), (255, 0, 0), 2, cv2.LINE_AA)  # Z
    ...
    def _eval_predictions(self):
            ...
            for file_name, gt_anno in obj_gts.items():
                ...
                if obj_name in cfg.DATASETS.SYM_OBJS:
                    ...
                    proj_2d_error = arp_2d(
                        R_pred, t_pred, R_gt_sym, t_gt, pts=self.models_3d[
                            cur_label]["pts"], K=gt_anno["K"]
                    )
                    ...
                    if "vsd" in metric_names:
                        # datasets/BOP_DATASETS/mp6d/data/0000/000204-color.png
                        depth_image_path = "/media/sda1/r10922190/BOP_DATASETS/" + \
                            '/'.join(file_name.split('/')[2:])
                        depth_image_path = depth_image_path.replace(
                            "color", "depth")
                        depth_image = cv2.imread(depth_image_path)
                        depth_image = depth_image[:, :, 0].T

                        vsd_error = vsd(R_pred, t_pred * 1000, R_gt, t_gt * 1000, depth_image, self.gts[obj_name][file_name]["K"], 15, [
                                        50], False, self.diameters[self.data_ref.obj2id[obj_name] - 1], self.ren, self.data_ref.obj2id[obj_name])
                        recalls[obj_name]["vsd"].append(
                            float(vsd_error[0]) < 0.3)
                ...
                ############
                recalls[obj_name]["ad_2"].append(
                    float(ad_error < 0.02 * self.diameters[cur_label]))
                recalls[obj_name]["ad_5"].append(
                ...
                recalls[obj_name]["rete_5"].append(
                    float(r_error < 5 and t_error < 0.05))
                recalls[obj_name]["rete_10"].append(
        ...
        recalls_cache_path = osp.join(
            self._output_dir, f"{method_name}_{self.dataset_name}_recalls.pkl")
        mmcv.dump(errors, errors_cache_path)
        mmcv.dump(recalls, recalls_cache_path)

        dump_tab_name = osp.join(
            self._output_dir, f"{method_name}_{self.dataset_name}_tab.txt")
    ...
    def _eval_predictions_precision(self):
        ...
        errors_cache_path = osp.join(
            self._output_dir, f"{method_name}_{self.dataset_name}_errors.pkl")
        recalls_cache_path = osp.join(
            self._output_dir, f"{method_name}_{self.dataset_name}_precisions.pkl")
        ...
        dump_tab_name = osp.join(
            self._output_dir, f"{method_name}_{self.dataset_name}_tab_precisions.txt")
        with open(dump_tab_name, "w") as f:
            f.write("{}\n".format(res_log_tab_str))
        if self._distributed:
            self._logger.warning(
